...
#AutoIt3Wrapper_Res_Fileversion=2.11.24.9
...
Func closeConnection($n)
	If $n > $max_connections Then
		ConsoleWrite("Encounter something Wrong!" & @CRLF)
		Return
	EndIf
...
;			Local $file = FileOpen($sourceFileName,16)	; open file for read only in binary mode
			Local $file = FileOpen($sourceFileName)	; open file for read only in default mode
...
Func processReply($n, $reply)
   ...
   Switch StringLower($msg[1])
	  ...
	  Case "file"
		 ...
		 $transFiles[$n] = FileOpen($destFileName,16+8+2)	; open file for  over-write in binary mode
		 $byteCounter[$n] = $len
		 pushCommand($n,"hold")
	  ...
	  Case "name"
		 ...
		 If $clientVersion <> $latestVersion Then
			$commands[$n] = "update C:\CopTraxTest\tmp\CopTraxTestClient.exe update C:\CopTraxTest\tmp\RestartClient.bat restart "
...
Func _Accept_Connection ()
	...
	For $i = 1 To $max_connections
		If $Sockets[$i] = 0 Then	;Find the first open socket.
			...
			logWrite(11, "Total " & $Sockets[0] & " clients now.")
			Return
		EndIf
	Next
