...
#pragma compile(FileVersion, 2.11.21.9)
...
Func _listenNewCommand()
	...
	Switch StringLower($Recv[1]) ; The last hotkey pressed.
		...
		Case "synctmz"
			...
			If ($Recv[0] >= 2) And _syncTime(StringTrimLeft($raw, 8)) Then
...
Func _syncTMZ($tmz)
	Local $t = StringSplit($tmz, "|")
	Local $t1 = Number($t[1])
	Local $t4 = Number($t[4])
	Local $t7 = Number($t[7])
	Local $t3 = _encodeSystemTime($t[3])
	Local $t6 = _encodeSystemTime($t[6])
	_logWrite("Trying to sync timezone " & $t1 & "," & $t[2] & "," & $t[3] & "," & $t4 & "," & $t[5] & "," & $t[6] & "," & $t7)
	Return _Date_Time_SetTimeZoneInformation($t1,$t[2],$t3,$t4,$t[5],$t6,$t7)
EndFunc
