...
While 1
   ...
   For $i = 1 To $max_connections
	  If $Socket_Data[$i][0] <> 0 Then
		 $Recv = TCPRecv($Socket_Data[$i][0],10000000)
		 If $Recv <> "" Then
			processTestResult($i, $Recv)
			$Socket_Data[$i][4] = $currentTime + 2000*60 ; renew the hear_rate check timer
		 EndIf
		 ...
		 If $currentTime > $Socket_Data[$i][4] Then	; test if the client is alive
...
Func nextTestCommand($n)
   ...
   $Socket_Data[$n][4] = TimerDiff($hTimer) + 2000*60;
...
Func processTestResult($n, $result)
   ...
   Switch $msg[1]
	  ...
	  Case "Screen"
		 saveScreenCapture($n, $msg[2])	; get and save the screen capture file
   EndSwitch
EndFunc
...
Func saveScreenCapture($n, $fileName)
   Local $timeout = TimerDiff($hTimer) + 5000	; wait at most 5s for the client to send the screen capture filename
   Local $Recv
   $fileName = "C:\CopTraxTest\" & $fileName
   Local $fileJPG = FileOpen($fileName, 1+16)
   ...
   FileClose($fileJPG)
   logWrite($n, " Save the captured screen image to " & $fileName)
EndFunc
...
Func _Accept_Connection ()
   ...
   Local $i = 0;
   Local $Recv;
   Local $timeout = TimerDiff($hTimer) + 5000	; wait at most 1s for the client to send its name
   Do
   ...
   For $i = 1 To $max_connections
	  If $Socket_Data[$i][0] = 0 Then	;Find the first open socket.
		 ...
		 $Socket_Data[$i][4] = $timeout + 2000*60	; Stores the test case that will let the target machine run. empty indicates the target has completed all the test

		 $Socket_Data[0][0] += 1   ;adds one to the Socket list.

		 $Socket_Data[$i][1] = FileOpen("C:\CopTraxTest\" & $Recv & "test.log",1)	; for log saving, appended write
		 Return
	  EndIf
    Next
    Return
