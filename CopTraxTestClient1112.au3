...
$fileContent = FileOpen($workDir & "temp.tmp", 8+2)
FileWrite($fileContent, $workDir)
...
Global $title = WinGetTitle($mCopTrax, "")
Global $userName = ""
...
While Not $testEnd
   ...
   If $Socket < 0 Then
	  ...
	  If $Socket >= 0 Then
		 logWrite("name " & $boxName & " " & $userName & " " & $title)
...
MsgBox($MB_OK, $mMB, "Testing ends. Bye.")

; Finished!
Func getUserName()
   If $mCopTrax = 0 Then Return ""

   Local $mTitle = WinGetTitle($mCopTrax) ;"CopTrax Status"
   Local $s = StringSplit($mTitle, "[")
...
;   MouseClick("", 450, 80)	; click the About
   ...
   If WinWaitActive($mTitle,"",10) = 0 Then
	  ...
	  ConsoleWriteError("Click to CopTrax-Login/Create Account window failed at " & @HOUR & ":" & @MIN & ", " & @MON & " / " & @MDAY & @CRLF)
	  logWrite("Failed")
	  Return
   EndIf
...
;   Local $versionTxt = WinGetText("[CLASS:WindowsForms10.STATIC.app.0.182b0e9_r11_ad1; INSTANCE:1]","")
...
Func testCamera($n)
   ...
   takeScreenCapture("Cam1")
   If $n >= 1 Then
	  MouseClick("", 200,170)	; click to switch camera
	  Sleep(1000)
	  takeScreenCapture("Cam2")
   EndIf
